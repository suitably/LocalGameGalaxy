...
export const LocalStoryEngine = {
  ...
  async updateGameDetails(
    ...
    if (Array.isArray(payload.players)) {
      const payloadPlayerMap = new Map(payload.players.map((p) => [p.id, p]));
      updatedPlayers = game.players.map((existingPlayer) => {
        const match = payloadPlayerMap.get(existingPlayer.id);
  ...
  async importSnapshot(
    ...
    const incomingPlayerMap = snapshot.game.players
      ? new Map(snapshot.game.players.map((p) => [p.id, p]))
      : undefined;
    ...
    if (hasPlayerChannelUpdates) {
      const mergedPlayers = existing.players.map((existingP) => {
        const incomingP = incomingPlayerMap?.get(existingP.id);
