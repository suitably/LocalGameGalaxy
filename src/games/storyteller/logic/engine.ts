...
export const LocalStoryEngine = {
  ...
  async updateGameDetails(
    ...
    if (Array.isArray(payload.players)) {
      const payloadPlayersMap = new Map(payload.players.map((p) => [p.id, p]));
      updatedPlayers = game.players.map((existingPlayer) => {
        const match = payloadPlayersMap.get(existingPlayer.id);
  ...
  async importSnapshot(
    ...
    const existingPlayersMap = new Map((existing.players || []).map((p) => [p.id, p]));

    if (isStorySnapshotNewer(snapshot, existing, existingEntries)) {
      const mergedPlayers = snapshot.game.players.map((incomingP) => {
        const existingP = existingPlayersMap.get(incomingP.id);
    ...
    if (hasPlayerChannelUpdates) {
      const snapshotPlayersMap = new Map((snapshot.game.players || []).map((p) => [p.id, p]));
      const mergedPlayers = existing.players.map((existingP) => {
        const incomingP = snapshotPlayersMap.get(existingP.id);
